class Vivarium < Formula
  desc "Local-first IMAP email sync for LLMs"
  homepage "https://github.com/ianzepp/vivarium"
  license "MIT"

  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/ianzepp/vivarium/releases/download/v6.0.0/vivi-aarch64-apple-darwin.tar.gz"
      sha256 "b6d249682934c1d1463d32fa7dbbeeb2f7e8dd59cddf219a4d2903e8cfdd7ec0"
    else
      url "https://github.com/ianzepp/vivarium/releases/download/v6.0.0/vivi-x86_64-apple-darwin.tar.gz"
      sha256 "fe8555048b954b8f09492c962d98388ca2362a6ee36bb5821abd9578ee63a871"
    end
  end

  def install
    bin.install "vivi", "vivi-pty"
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/vivi --version")
    assert_match version.to_s, shell_output("#{bin}/vivi-pty --version")
    assert_match "exec", shell_output("#{bin}/vivi --help")
    assert_match "agent", shell_output("#{bin}/vivi --help")
    assert_match "poll", shell_output("#{bin}/vivi agent --help")
    assert_match "send", shell_output("#{bin}/vivi exec --help")
  end
end
